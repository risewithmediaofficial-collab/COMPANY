    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        ...
        <div className="space-y-5 pt-2">
          ...
          {/* Video Shoot Content & Reels Progress */}
          {isVideoShoot ? (
            <div className="grid gap-3 sm:grid-cols-2 p-4 rounded-2xl border border-border/60 bg-card">
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs font-semibold">
                  ...
                  <span className="text-primary">{item.completedContents || 0} / {item.plannedContents || 0} ({item.contentsCompletionPct || 0}%)</span>
                </div>
                <div className="h-2 w-full bg-secondary rounded-full overflow-hidden">
                  <div className="h-full bg-primary" style={{ width: `${Math.min(item.contentsCompletionPct || 0, 100)}%` }} />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs font-semibold">
                  ...
                  <span className="text-emerald-500">{item.completedReels || 0} / {item.plannedReels || 0} ({item.reelsCompletionPct || 0}%)</span>
                </div>
                <div className="h-2 w-full bg-secondary rounded-full overflow-hidden">
                  <div className="h-full bg-emerald-500" style={{ width: `${Math.min(item.reelsCompletionPct || 0, 100)}%` }} />
                </div>
              </div>
            </div>
          ) : null}
          ...
          {/* Notes */}
          {item.notes || item.promotionDetails || item.remarks ? (
            <div className="rounded-2xl border border-border/60 bg-card p-4 space-y-1">
              <h4 className="text-xs font-bold text-foreground">Notes / Details</h4>
